...
$sql_my_posts = "SELECT post_id, id, title, content, images FROM Posts WHERE id = ? AND banned = 0 ORDER BY created_at DESC";

// Array to store user's posts
$my_posts = array();

$stmt = $conn->prepare($sql_my_posts);
...
$stmt->execute();
$stmt->bind_result($post_id, $user_id, $title, $content, $images);
...
<head>
    ...
    <title><?php echo htmlspecialchars($username); ?>'s Posts | Land Life Community</title>
    ...
    <div class="form">
        ...
        <div class="posts-section">
            <h2 class="section-title">
                <i class="fas fa-pen-alt"></i> My Posts
            </h2>
            
            <?php if (!empty($my_posts)): // Check whether have post or not?>
                <?php foreach ($my_posts as $post): // Loop through each post?>
                    <div class="post">
                        <!-- Title -->
                        ...
                        <a href="view_post.php?post_id=<?php echo $post['post_id']; ?>" class="view-link">
