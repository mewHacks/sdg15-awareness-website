...
// Get liked posts data from database
$sql_liked_posts = "
    SELECT Posts.post_id, Posts.title, Posts.content, Posts.images 
    FROM Posts
    JOIN Likes ON Likes.post_id = posts.post_id
    WHERE Likes.id = ?
    ORDER BY Likes.created_at DESC";

// Array to store liked posts
$liked_posts = array();
...
$stmt->execute();
$stmt->bind_result($id, $title, $content, $images);
...
<head>
    ...
    <title><?php echo htmlspecialchars($username); ?>'s Liked Posts | Land Life Community</title>
...
<body>
    ...
    <div class="form">
        ...
        <div class="profile-header">
            <h1 class="profile-title">Liked Posts</h1>
        ...
        <div class="liked-section">
            ...
            <?php if (!empty($liked_posts)): // Check whether have post or not?>
                <?php foreach ($liked_posts as $post): // Loop through each post?>
                    <div class="post">
                        <!-- Title -->
                        ...
                        <a href="view_post.php?post_id=<?php echo $post['id']; ?>" class="view-link">
